...
describe("EditorComponent", () => {
	...
	it("calls the service with title, description, body from the form", () => {
		mockAiTagService.suggestTags.mockReturnValue(
			of(["angular", "typescript", "rxjs"]),
		);
		...
		expect(mockAiTagService.suggestTags).toHaveBeenCalledWith(
			"My Title",
			"My Desc",
			"My Body",
		);
	...
	it("Suggest Tags button is disabled when title or body is empty", () => {
		...
		fixture.detectChanges();
		expect(
			getSuggestButton(fixture.nativeElement as HTMLElement)?.disabled,
		).toBe(true);
	...
	it("renders suggested tags as buttons after service responds", () => {
		...
		const pills = el.querySelectorAll<HTMLButtonElement>(
...
function setInputValue(
	...
	const el = nativeElement.querySelector<
